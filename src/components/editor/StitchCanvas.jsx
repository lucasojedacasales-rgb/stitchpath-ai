...
/**
 * StitchCanvas - dibuja region.stitches [{x,y}] en mm directamente en canvas.
 * Sin path_points, sin clip — dibuja cada punto/línea de puntada real.
 */
export default function StitchCanvas({
  ...
  const [, forceRender]     = useState(0); // trigger redraws
  ...
  // ── Resize canvas elements to match container ──────────────────────────────
  function syncSize() {
  ...
  useEffect(() => {
    const obs = new ResizeObserver(() => { syncSize(); forceRender(n => n + 1); });
  ...
  useEffect(() => {
    ...
    img.onload = () => { imageRef.current = img; forceRender(n => n + 1); };
    img.onerror = () => {
      const img2 = new Image();
      img2.onload = () => { imageRef.current = img2; forceRender(n => n + 1); };
      img2.src = imageUrl;
    };
    img.src = imageUrl;
  }, [imageUrl]);
  ...
  useEffect(() => {
    ...
    for (const region of regions) {
      ...
      ctx.save();
      ctx.globalAlpha = alpha * (isSelected ? 1.0 : 0.8);
  ...
  return (
    <div className="relative flex flex-col w-full h-full bg-[#0d0f14]">
      ...
      <div
        ref={containerRef}
        className="relative flex-1 overflow-hidden stitch-canvas-grid"
      ...
      >
        ...
        <canvas ref={stitchCanvasRef} className="absolute inset-0 w-full h-full" style={{ pointerEvents: 'none' }} />
      </div>
    </div>
  );
}
