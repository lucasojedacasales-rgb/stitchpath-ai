...
export default function RegionEditModal({ region, onSave, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
      <div className="bg-[#161a23] border border-[#2a2d3a] rounded-xl w-80 max-h-[90vh] overflow-y-auto shadow-2xl">
        ...
        <div className="p-5 space-y-4">
          {/* Color */}
          <div>
            <label className="text-[11px] text-slate-500 uppercase tracking-wider mb-2 block">Color de hilo</label>
          ...
          {/* Stitch type */}
          <div>
            <label className="text-[11px] text-slate-500 uppercase tracking-wider mb-2 block">Tipo de puntada</label>
            <div className="grid grid-cols-3 gap-1.5">
              {[['fill', 'Fill'], ['satin', 'Satin'], ['running_stitch', 'Running']].map(([val, label]) => (
                <button
                  key={val} onClick={() => set('stitch_type', val)}
          ...
          <div>
            ...
            <input type="number" min="1" max="20" value={r.layer_order || 1} onChange={e => set('layer_order', Number(e.target.value))}
          ...
          <div className="flex items-center justify-between">
            <span className="text-[11px] text-slate-400">Underlay</span>
            <button
              onClick={() => set('underlay', !r.underlay)}
              className={`relative w-9 h-5 rounded-full transition-colors ${r.underlay ? 'bg-violet-600' : 'bg-[#2a2d3a]'}`}
            >
              <div className={`absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-white transition-transform ${r.underlay ? 'translate-x-4' : ''}`} />
            </button>
          </div>

          {/* Presets */}
          <div className="border-t border-[#1e2130] pt-4">
            <p className="text-[11px] text-slate-500 uppercase tracking-wider mb-2">Aplicar Preset</p>
