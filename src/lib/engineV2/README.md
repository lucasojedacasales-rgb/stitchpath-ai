...
## Phase 13A1: unresolved-review policy gate

Phase 13A1 evaluates the existing proposal-review and draft-materialization result without changing either algorithm. A deferred or blocked review decision is an intermediate policy block at `draft_materialization`; the direct materialization result remains available for diagnostics, while thread resolution and every later stage are skipped. If some drafts exist while any proposal remains unresolved, the whole transaction blocks so an incomplete accepted subset cannot be exported.

Explicit `exclude` and `reject` decisions are final dispositions. When all final decisions leave no stitchable drafts, the pipeline blocks explicitly with `NO_STITCHABLE_PROPOSALS_AFTER_REVIEW` instead of compiling an empty END-only stream. An accepted or overridden active proposal without a corresponding draft is classified as an internal validation failure. A fully resolved design with at least one draft continues unchanged.

The orchestrator does not infer semantic roles, fabricate manual-review decisions, lower review thresholds, or alter proposal, draft, region, geometry, hole, or color data. DSB strict trim policy remains a distinct final-stage completed `policy_blocked` outcome; it is not converted into an intermediate pipeline block. V2 remains disconnected from the application and Base44, and V1 remains untouched.
