...
describe('Phase 9 polygon scanline clipper', () => {
  ...
  it('splits one row into three material intervals', () => { const second = hole.map(point => ({ x: point.x + 3, y: point.y })); expect(clip({ holes: [hole, second] }).intervals.map(interval => [interval.start.x, interval.end.x])).toEqual([[0, 4], [6, 7], [9, 10]]); });
  it('treats opposite source ring windings as equivalent explicit material roles', () => { const forward = clip({ holes: [hole] }); const reversed = clip({ outerPolygon: [...rectangle].reverse(), holes: [[...hole].reverse()] }); expect(reversed.intervals).toEqual(forward.intervals); });
  it('ignores a protected hole that touches no scan row', () => { const distant = hole.map(point => ({ x: point.x, y: point.y + 5 })); expect(clip({ holes: [distant] }).intervals).toEqual(clip().intervals); });
  it('subtracts the union of overlapping hole intervals deterministically', () => { const overlapping = [{ x: 5, y: 3 }, { x: 8, y: 3 }, { x: 8, y: 7 }, { x: 5, y: 7 }]; expect(clip({ holes: [hole, overlapping] }).intervals.map(interval => [interval.start.x, interval.end.x])).toEqual([[0, 4], [8, 10]]); });
  it('sorts compound material intervals deterministically regardless of hole order', () => { const second = hole.map(point => ({ x: point.x + 3, y: point.y })); expect(clip({ holes: [second, hole] }).intervals.map(interval => [interval.start.x, interval.end.x])).toEqual(clip({ holes: [hole, second] }).intervals.map(interval => [interval.start.x, interval.end.x])); });
