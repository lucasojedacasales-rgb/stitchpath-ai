...
export function createSyntheticDSTMachineStream(specs = [], options = {}) {
  const profile = createMachineProfileV2({ ...GENERIC_DST_MACHINE_PROFILE, id: options.profileId || 'synthetic-dst-format', coordinateResolutionMm: options.resolution ?? 0.1, initialMachinePositionUnits: options.initialMachinePositionUnits || { x: 0, y: 0 } });
  ...
  const commands = specs.map((spec, adaptedIndex) => {
    ...
    return createMachineAdaptedCommandV2({
      ...
      sourceCanonicalCommandId: `canonical:synthetic:${String(adaptedIndex).padStart(4, '0')}`,
      sourceCanonicalCommandIndex: adaptedIndex, splitIndex: 0, splitCount: 1, quantizationErrorMm: 0,
      reasonCode: spec.reasonCode || 'SYNTHETIC_DST_FIXTURE', source: { fixture: options.fixtureId || 'dst-basic' },
    });
  });
  const spans = commands.map((command, index) => createCanonicalCommandAdaptationSpanV2({ canonicalCommandId: command.sourceCanonicalCommandId, canonicalCommandIndex: index, status: 'adapted', firstAdaptedCommandIndex: index, lastAdaptedCommandIndex: index, adaptedCommandCount: 1, splitApplied: false, quantizationApplied: ['stitch', 'jump'].includes(command.type), source: { fixture: options.fixtureId || 'dst-basic' } }));
...
export function createDSTBasicFormatFixture(type = 'stitch') {
  return createSyntheticDSTMachineStream([{ type, dxUnits: 10, dyUnits: -5 }, { type: 'end' }], { fixtureId: `dst-basic-${type}` });
}
