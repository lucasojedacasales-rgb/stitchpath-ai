    ...
    try {
      ...
      if (res.data?.success) {
        ...
        const classifyStitchType = (region) => {
          ...
          if (area > 200 && avgWidth > 5.0) return 'fill';
          else if (area < 50 || avgWidth < 3.0 || compactness > 15) return 'satin';
          else return 'fill';
        };
        ...
        const calculateStitchCount = (region) => {
          ...
          if (type === 'fill') return Math.round(area * density * 2.5);
          else if (type === 'satin') {
            const width = Math.max(1, area / perim);
            return Math.round((perim / 2.5) * (width / Math.max(0.4, density)));
          } else return Math.round(perim / 1.5);
        };
        ...
        const newRegions = filtered.map((r) => {
          ...
          return {
            ...r,
            name: r.name || `${position}_${colorName}_${typeAbbr}`,
            stitch_type: type,
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          {imageUrl && regions.length === 0 && !processing && (
            <div className="border-t border-[#1a1d27] p-3 flex items-center gap-3 bg-[#0a0c12]">
              ...
              <button onClick={startProcessing} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-violet-600 hover:bg-violet-500 text-white text-xs font-bold transition-colors">
...
function ProjectNameInput({ name, onSave }) {
  ...
  if (editing) return (
    <input autoFocus value={val} onChange={e => setVal(e.target.value)} onBlur={() => { onSave(val); setEditing(false); }} onKeyDown={e => { if (e.key === 'Enter') { onSave(val); setEditing(false); } if (e.key === 'Escape') setEditing(false); }}
...
function NavButton({ onClick, icon: Icon, label, accent, disabled }) {
  return (
    <button onClick={onClick} disabled={disabled}
      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors disabled:opacity-40 disabled:cursor-not-allowed ${accent ? 'bg-violet-600 hover:bg-violet-500 text-white' : 'bg-[#161a23] border border-[#2a2d3a] text-slate-400 hover:text-white hover:bg-[#1e2130]'}`}>
...
function SliderControl({ label, value, onChange, color }) {
  return (
    <div className="flex items-center gap-2">
      ...
      <input type="range" min="0" max="100" value={value} onChange={e => onChange(Number(e.target.value))} className="w-20 accent-violet-600" />
...
function FilterToggle({ label, active, onChange, color }) {
  ...
  return (
    <button onClick={() => onChange(!active)} className={`text-[10px] px-2 py-1 rounded border transition-colors font-medium ${active ? accent : 'border-[#2a2d3a] text-slate-600 hover:text-slate-400'}`}>
      {label}
    </button>
  );
}
...
function UploadZone({ onUpload, fileInputRef, uploading }) {
  ...
  return (
    <div className={`flex-1 flex items-center justify-center border-2 border-dashed transition-colors m-6 rounded-2xl cursor-pointer ${dragOver ? 'border-violet-500 bg-violet-900/10' : 'border-[#2a2d3a] hover:border-violet-500/50'}`}
